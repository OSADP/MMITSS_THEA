...
CopDualRing::CopDualRing(MmitssSignalControl& parent)
    : m_parent(parent)
    , Ped_Phase_Considered(m_parent.m_mib.getPedPhaseConsidered())
    , MaxGreen(m_parent.m_maxGreen)
    , MinGreen(m_parent.m_minGreen)
    , ArrivalTable(m_parent.m_arrivalTable)
    , delay_type(m_parent.m_delayType)
    , queue_warning_P1(m_parent.m_queueWarningP1)
    , queue_warning_P5(m_parent.m_queueWarningP5)
    , PedWalk(m_parent.m_pedWalk)
    , PedClr(m_parent.m_pedClr)
    , opt_sig_plan(m_parent.m_optSigPlan)
    , opt_sig_seq(m_parent.m_optSigSeq)
    , LaneNo(m_parent.m_laneNo)
    , red_elapse_time(m_parent.m_redElapseTime)
    , yellow(m_parent.m_yellow)
    , red(m_parent.m_red)
{
}

int CopDualRing::COP_DUAL_RING(
    int InitPhase[2], int GrnElapse[2], int PassedPhase[2], int Skip_phase[8])
{
    ...
    // Skip_phase[0]=1;Skip_phase[1]=1;Skip_phase[2]=1;Skip_phase[3]=1;Skip_phase[4]=1;Skip_phase[5]=1;Skip_phase[6]=1;Skip_phase[7]=1;
    // //no phase skipped

    int start_barrier;

    ITSAPP_TRACE("The Delay Type is: %d", delay_type);
    ITSAPP_TRACE("Ped Phase needs to be considered: %d %d %d %d %d %d %d %d\n",
        Ped_Phase_Considered[0], Ped_Phase_Considered[1], Ped_Phase_Considered[2],
        Ped_Phase_Considered[3], Ped_Phase_Considered[4], Ped_Phase_Considered[5],
        Ped_Phase_Considered[6], Ped_Phase_Considered[7]);
    ...
    // consider the Ped constraint, if the ped phase is on, or called, then when planning, MinGreen
    // should be changed
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        if (PassedPhase[j] != 0)
        {
            planned_phase[j][PassedPhase[j] - start_barrier * 2 - j * 4 - 1] =
                0; // skip the passed phase
        }
        planned_phase[j][InitPhase[j] - start_barrier * 2 - j * 4 - 1] =
            InitPhase[j]; // the phase already started can't be skipped even if no call
    }

    // sprintf(temp_log,"The planned phases are: %d %d %d
    ...
    for (i = 0; i < 2; i++) // ring 1
    {
        if (planned_phase[0][i] != 0 &&
            planned_phase[0][i] != InitPhase[0]) // not the current phase, but in plan
        {
            ring1_min += MinGreen_Ped[planned_phase[0][i] - 1] + yellow[planned_phase[0][i] - 1] +
                         red[planned_phase[0][i] - 1];
            ring1_max += MaxGreen[planned_phase[0][i] - 1] + yellow[planned_phase[0][i] - 1] +
                         red[planned_phase[0][i] - 1];
        }
        if (planned_phase[0][i] != 0 &&
            planned_phase[0][i] == InitPhase[0]) // the current phase in plan
        {
            ring1_min += yellow[planned_phase[0][i] - 1] + red[planned_phase[0][i] - 1] +
                         max(0, MinGreen_Ped[planned_phase[0][i] - 1] - GrnElapse[0]);
            ring1_max += MaxGreen[planned_phase[0][i] - 1] - GrnElapse[0] +
                         yellow[planned_phase[0][i] - 1] + red[planned_phase[0][i] - 1];
        }
    }
    for (i = 0; i < 2; i++) // ring 2
    {
        ...
        {
            ring2_min += MinGreen_Ped[planned_phase[1][i] - 1] + yellow[planned_phase[1][i] - 1] +
                         red[planned_phase[1][i] - 1];
            ring2_max += MaxGreen[planned_phase[1][i] - 1] + yellow[planned_phase[1][i] - 1] +
                         red[planned_phase[1][i] - 1];
        }
        if (planned_phase[1][i] != 0 &&
            planned_phase[1][i] == InitPhase[1]) // the current phase in plan
        {
            ring2_min += yellow[planned_phase[1][i] - 1] + red[planned_phase[1][i] - 1] +
                         max(0, MinGreen_Ped[planned_phase[1][i] - 1] - GrnElapse[1]);
            ring2_max += MaxGreen[planned_phase[1][i] - 1] - GrnElapse[1] +
                         yellow[planned_phase[1][i] - 1] + red[planned_phase[1][i] - 1];
        }
    }
    B_Min[stage] = max(ring1_min, ring2_min);
    B_Max[stage] = min(ring1_max, ring2_max); // if one ring exceeds the maximum green time, just
                                              // put the last phase to green rest!!!!!
    ...
    for (i = 1; i < TOTAL_TIME; i++)
    {
        ...
        if (B_Min[stage] <= i && B_Max[stage] >= i) // feasible
        {
            ...
            for (j = 0; j < 2; j++)
                for (k = 0; k < 2; k++)
                {
                    if (planned_phase[j][k] != 0)
                        effectiveG[j] -=
                            yellow[planned_phase[j][k] - 1] + red[planned_phase[j][k] - 1];
                }
            // Add another constraint: if the other ring only have one phase, this the sum of two
            // phases of this ring shouldn't exceed the maximum green of the phase in the other ring

            // Calculate Number of Vehicle in each phase until time i;
            int Num_veh[8] = {0}; // Number of vehicles each phase
            int total_num_veh = 0;
            float Num_Veh_Ratio[8] = {
                0}; // radio of vehicles e.g. ratio=Num_veh[i]/(sum(Num_veh[i]);
            for (int ii = 0; ii < NUM_PHASES; ii++)
            {
            ...
            for (k = 0; k < 2; k++) // two rings
            {
                ...
                {
                    // the initial phase can ternimate at any time, so start from 0 to
                    // Gmax-GrnElapse
                    if (planned_phase[k][0] == InitPhase[k])
                    {
                        float delay_other_phase =
                            f_other(k, planned_phase[k][0], planned_phase[k][1], i, Num_veh,
                                Num_Veh_Ratio); // calculate the delay of other phases in the same
                                                // ring rather than planned phase
                        for (j = max(0, MinGreen_Ped[planned_phase[k][0] - 1] - GrnElapse[k]);
                             j <= MaxGreen[planned_phase[k][0] - 1] - GrnElapse[k]; j++)
                        {
                            tempG[k][0] = j;
                            tempG[k][1] = effectiveG[k] - tempG[k][0];
                            if (tempG[k][1] >= MinGreen_Ped[planned_phase[k][1] - 1] &&
                                tempG[k][1] <=
                                    MaxGreen[planned_phase[k][1] - 1]) // feasible solution
                            {
                                // calculate delay function produced by this
                                // combination!!!!!!!!!!!!!!!!!!!!
                                float temp_vv =
                                    f(k, planned_phase[k][0], planned_phase[k][1], tempG[k][0],
                                        tempG[k][1], i, Num_veh, Num_Veh_Ratio) +
                                    delay_other_phase;
                    ...
                    {
                        // calculate the delay of other phases in the same ring rather than planned
                        // phase
                        float delay_other_phase = f_other(
                            k, planned_phase[k][1], planned_phase[k][0], i, Num_veh, Num_Veh_Ratio);
                        for (j = max(0, MinGreen_Ped[planned_phase[k][1] - 1] - GrnElapse[k]);
                             j <= MaxGreen[planned_phase[k][1] - 1] - GrnElapse[k]; j++)
                        {
                            tempG[k][1] = j;
                            tempG[k][0] = effectiveG[k] - tempG[k][1];
                            if (tempG[k][0] >= MinGreen_Ped[planned_phase[k][0] - 1] &&
                                tempG[k][0] <=
                                    MaxGreen[planned_phase[k][0] - 1]) // feasible solution
                            {
                                // calculate delay function produced by this
                                // combination!!!!!!!!!!!!!!!!!!!!
                                float temp_vv =
                                    f(k, planned_phase[k][1], planned_phase[k][0], tempG[k][1],
                                        tempG[k][0], i, Num_veh, Num_Veh_Ratio) +
                                    delay_other_phase;
                                if (temp_vv < temp_v[k]) // calculate the delay
                                {
                                    temp_v[k] = temp_vv; // save the best delay
                ...
                if (planned_phase[k][0] != 0 && planned_phase[k][1] == 0)
                {
                    ...
                    {
                        temp_v[k] = f_r(k, planned_phase[k][0], planned_phase[k][1], tempG[k][0],
                            tempG[k][1], i, Num_veh, Num_Veh_Ratio);
                        // save the best phase schedule
                        best_phase_schedule[stage][i][k][0] = tempG[k][0];
                        best_phase_schedule[stage][i][k][1] = tempG[k][1];
    ...
    ITSAPP_TRACE("start of the later stages");

    // 2. Construct the second and third stage barrier

    for (stage = 1; stage < NUM_BARRIERS; stage++)
    {
        // advance to another barrier
        ITSAPP_TRACE("Stage %d", stage);
        ...
        if (planned_phase[0][0] != 0 || planned_phase[0][1] != 0 || planned_phase[1][0] != 0 ||
            planned_phase[1][1] != 0) // either one of the phases is not zero
        ...
        for (i = 0; i < 2; i++) // ring 1
        {
            if (planned_phase[0][i] != 0)
            {
                ring1_min += MinGreen_Ped[planned_phase[0][i] - 1] +
                             yellow[planned_phase[0][i] - 1] + red[planned_phase[0][i] - 1];
                ring1_max += MaxGreen[planned_phase[0][i] - 1] + yellow[planned_phase[0][i] - 1] +
                             red[planned_phase[0][i] - 1];
            }
        ...
        for (i = 0; i < 2; i++) // ring 2
        {
            if (planned_phase[1][i] != 0)
            {
                ring2_min += MinGreen_Ped[planned_phase[1][i] - 1] +
                             yellow[planned_phase[1][i] - 1] + red[planned_phase[1][i] - 1];
                ring2_max += MaxGreen[planned_phase[1][i] - 1] + yellow[planned_phase[1][i] - 1] +
                             red[planned_phase[1][i] - 1];
            }
        ...
        B_Min[stage] = max(ring1_min, ring2_min);
        B_Max[stage] = min(ring1_max, ring2_max); // if one ring exceeds the maximum green time,
                                                  // just put the last phase to green rest!!!!!

        int min_cons = 0;
        int max_cons =
            B_Max[stage]; // minimum, and maximum constrain of the start and end time of the phase

        for (i = 0; i < stage; i++)
        {
            min_cons += B_Min[i];
            max_cons += B_Max[i];
            max_cons = min(max_cons,
                TOTAL_TIME -
                    1); // max time should be constrained by the total amount to time allocated
        }

        int s;
        int temp_X;

        // Variable to same value of the performance function, with same residual queue, same green
        // length and same phase sequence
        float* perf_func_value_seq1;
        perf_func_value_seq1 =
            new float[5120000]; // 2*40*40*40*40    ring*residual Q1*residual Q2*g1*g2
        ...
        for (i = 1; i < TOTAL_TIME; i++)
        {
            if (i >= min_cons + B_Min[stage] && i <= max_cons)
            {
                ...
                int temp_Q_perm[8];
                for (s = max(B_Min[stage], i - B_Max[stage - 1]);
                     s <= min(i - min_cons, B_Max[stage]); s++)
                {

                    // Calculate Number of Vehicle in each phase until time between s and i= arrival
                    // + residual queue at s-i
                    ...
                    // sprintf(temp_log,"Num Vehicle is: %d %d %d %d %d %d %d
                    ...
                    // cout<<"Barrier length is: "<<i<<endl;
                    // calculate the effective total green time, just length - Y - R of each planned
                    // phase
                    effectiveG[0] = s;
                    effectiveG[1] = s;
                    for (j = 0; j < 2; j++)
                    ...
                    for (k = 0; k < 2; k++) // two rings
                    {
                        ...
                        if (planned_phase[k][0] != 0 && planned_phase[k][1] != 0)
                        {
                            float delay_other_phase = f1_other(k, planned_phase[k][0],
                                planned_phase[k][1], i, s, stage, Num_veh, Num_Veh_Ratio);
                            for (j = MinGreen_Ped[planned_phase[k][0] - 1];
                                 j <= MaxGreen[planned_phase[k][0] - 1]; j++) // phase sequence 1
                            {
                                tempG[k][0] = j;
                                tempG[k][1] = effectiveG[k] - tempG[k][0];
                                if (tempG[k][1] >= MinGreen_Ped[planned_phase[k][1] - 1] &&
                                        ...
                                        MaxGreen[planned_phase[k][1] - 1]) // feasible solution
                                {
                                    ...
                                    float temp_vv;
                                    int Q_temp_P1 =
                                        Q_perm[stage - 1][i - s][planned_phase[k][0] - 1];
                                    int Q_temp_P2 =
                                        Q_perm[stage - 1][i - s][planned_phase[k][1] - 1];
                                    int array_position = k * 2560000 + Q_temp_P1 * 64000 +
                                                         Q_temp_P2 * 1600 + tempG[k][0] * 40 +
                                                         tempG[k][1] - 1;
                                    if (Q_temp_P1 <= 40 && Q_temp_P2 <= 40 && tempG[k][0] <= 40 &&
                                        tempG[k][1] <= 40 &&
                                        i - s > 30) // make sure all the values are in the boundary
                                                    // of the matrix definition, and the ARRIVAL
                                                    // TABLE IS THE SAME
                                    {
                                        if (perf_func_value_seq1[array_position] <
                                            99999) // already calculated
                                        {
                                            temp_vv = perf_func_value_seq1[array_position] +
                                                      delay_other_phase;
                                        ...
                                        {
                                            int f1_value = f1(k, planned_phase[k][0],
                                                planned_phase[k][1], tempG[k][0], tempG[k][1], i, s,
                                                stage, Num_veh, Num_Veh_Ratio);
                                    ...
                                    {
                                        temp_vv = f1(k, planned_phase[k][0], planned_phase[k][1],
                                                      tempG[k][0], tempG[k][1], i, s, stage,
                                                      Num_veh, Num_Veh_Ratio) +
                                                  delay_other_phase;
                            ...
                            for (j = MinGreen_Ped[planned_phase[k][1] - 1];
                                 j <= MaxGreen[planned_phase[k][1] - 1];
                                 j++) // switch phase sequence
                            {
                                tempG[k][1] = j;
                                tempG[k][0] = effectiveG[k] - tempG[k][1];
                                if (tempG[k][0] >= MinGreen_Ped[planned_phase[k][0] - 1] &&
                                        ...
                                        MaxGreen[planned_phase[k][0] - 1]) // feasible solution
                                {
                                    ...
                                    int array_position = k * 2560000 + Q_temp_P1 * 64000 +
                                                         Q_temp_P2 * 1600 + tempG[k][1] * 40 +
                                                         tempG[k][0] - 1;
                                    // cout<<perf_func_value_seq2[array_position]<<endl;
                                    if (Q_temp_P1 <= 40 && Q_temp_P2 <= 40 && tempG[k][0] <= 40 &&
                                        tempG[k][1] <= 40 && i - s > 30) // make sure all the values
                                                                         // are in the boundary of
                                                                         // the matrix definition
                                    {
                                        if (perf_func_value_seq2[array_position] <
                                            99999) // already calculated
                                        {
                                            temp_vv = perf_func_value_seq2[array_position] +
                                                      delay_other_phase;
                                            // count++;
                                        }
                                        else
                                        {
                                            int f1_value = f1(k, planned_phase[k][1],
                                                planned_phase[k][0], tempG[k][1], tempG[k][0], i, s,
                                                stage, Num_veh, Num_Veh_Ratio);
                                            temp_vv = f1_value + delay_other_phase;
                                    ...
                                    {
                                        temp_vv = f1(k, planned_phase[k][1], planned_phase[k][0],
                                                      tempG[k][1], tempG[k][0], i, s, stage,
                                                      Num_veh, Num_Veh_Ratio) +
                                                  delay_other_phase;
                        ...
                        if (planned_phase[k][0] != 0 && planned_phase[k][1] == 0)
                        {
                            ...
                            if (tempG[k][0] <= MaxGreen[planned_phase[k][0] - 1])
                            {
                                temp_v[k] = f1_r(k, planned_phase[k][0], planned_phase[k][1],
                                    tempG[k][0], tempG[k][1], i, s, stage, Num_veh, Num_Veh_Ratio);
                                // save the best phase schedule
                                opt_schedule[k][0] = tempG[k][0];
                                opt_schedule[k][1] = tempG[k][1];
                                opt_seq[k][0] = planned_phase[k][0];
                    ...
                    temp_total_v =
                        temp_v[0] + temp_v[1] +
                        v[stage - 1][i - s]; // delay of this stage + delay at previous stage
                    ...
                    if (temp_total_v < v[stage][i])
                    {
                        ...
                        for (int kkk = 0; kkk < NUM_PHASES; kkk++)
                            Q_perm[stage][i][kkk] = temp_Q_perm[kkk];

                        // assign permenent queue
                    }
                }
                // because of the min constraint, it is possible that planning the third stage
                // result in worse consequence
                /*
    ...
    if (B_Min[NUM_BARRIERS - 1] == 0 || B_Max[NUM_BARRIERS - 1] == 0)
    {
        stage = NUM_BARRIERS - 2;
        start_stage = NUM_BARRIERS - 2;
    }
    else
    {
        stage = NUM_BARRIERS - 1;
        start_stage = NUM_BARRIERS - 1;
    }
    ...
    // find out hte place to trace back where have the changed residual queue and shortest time
    // horizon
    ...
    int start_time = min(TOTAL_TIME - 1, best_position);
    int time = min(TOTAL_TIME - 1, best_position);
    ...
    //     for(j=0;j<121;j++)
    //     {
    //         sprintf(temp_log,"perminent queue at stage %d and time %d is %d %d %d %d %d %d %d
    //                 %d\n",i,j,Q_perm[i][j][0],Q_perm[i][j][1],Q_perm[i][j][2],Q_perm[i][j][3],
    //                 Q_perm[i][j][4],Q_perm[i][j][5],Q_perm[i][j][6],Q_perm[i][j][7]);
    //         outputlog(temp_log); cout<<temp_log;
    //     }
    // }

    ITSAPP_TRACE("perminent queue at start_time-1 is %d %d %d %d %d %d %d %d",
        Q_perm[NUM_BARRIERS - 1][start_time - 1][0], Q_perm[NUM_BARRIERS - 1][start_time - 1][1],
        Q_perm[NUM_BARRIERS - 1][start_time - 1][2], Q_perm[NUM_BARRIERS - 1][start_time - 1][3],
        Q_perm[NUM_BARRIERS - 1][start_time - 1][4], Q_perm[NUM_BARRIERS - 1][start_time - 1][5],
        Q_perm[NUM_BARRIERS - 1][start_time - 1][6], Q_perm[NUM_BARRIERS - 1][start_time - 1][7]);
    ITSAPP_TRACE("Start time is %d", start_time);
    ITSAPP_TRACE("Optimal Planning time is %d %d", opt_s[0], opt_s[1]);
    ...
    if (start_stage == NUM_BARRIERS - 1)
    {
        for (i = 0; i < 2; i++)
            for (j = 0; j < 2; j++)
            {
                ...
                ITSAPP_TRACE("1: opt_sig_plan[%d][%d]: %d", i, j, opt_sig_plan[i][j]);
                ITSAPP_TRACE("opt_sig_seq[%d][%d]: %d", i, j, opt_sig_seq[i][j]);
            }
        // second barrier
        for (i = 0; i < 2; i++)
            for (j = 0; j < 2; j++)
            {
                ...
                ITSAPP_TRACE("2: opt_sig_plan[%d][%d]: %d", i, j, opt_sig_plan[i][j]);
                ITSAPP_TRACE("opt_sig_seq[%d][%d]: %d", i, j, opt_sig_seq[i][j]);
            }
    }

    if (start_stage == NUM_BARRIERS - 2)
    {
        for (i = 0; i < 2; i++)
            for (j = 0; j < 2; j++)
            {
                ...
                ITSAPP_TRACE("3: opt_sig_plan[%d][%d]: %d", i, j, opt_sig_plan[i][j]);
                ITSAPP_TRACE("opt_sig_seq[%d][%d]: %d", i, j, opt_sig_seq[i][j]);
            }
        // second barrier
        for (i = 0; i < 2; i++)
            for (j = 0; j < 2; j++)
            {
                ...
                opt_sig_seq[i][j + 2] = 0;
                ITSAPP_TRACE("4: opt_sig_plan[%d][%d]: %d", i, j, opt_sig_plan[i][j]);
                ITSAPP_TRACE("opt_sig_seq[%d][%d]: %d", i, j, opt_sig_seq[i][j]);
            }
    }
...
float CopDualRing::f_r(int ring,
    int phase1,
    int phase2,
    int g1,
    int g2,
    ...
    if (phase1 == 0)
    {
        for (i = 1; i <= total_length; i++)
        {
            if (i <= g2) // phase 2 is green
            {
                ...
                {
                    Q_temp[i][phase2 - 1] -= LaneNo[phase2 - 1]; // discharging rate is based on the
                                                                 // number of the lane, assuming
                                                                 // queue at each lane are equal

                    // if (phase2==2 || phase2==6)
                    // Q_temp[i][phase2-1]-=3;  //queue discharge rate is 3 veh/2second for through
                    // phase 2 and 6, 2 veh/2second for phase 4 and 8, lanes and 1 veh/2 second for
                    // left turn phases
                ...
                if (Q_temp[i][phase2 - 1] <= 0)
                    Q_temp[i][phase2 - 1] =
                        0; // if queue is 0, the arrival flow can pass, no need to add
                else
    ...
    if (phase2 == 0)
    {
        for (i = 1; i <= total_length; i++)
        {
            if (i <= g1) // phase 2 is green
            {
                ...
                if ((i + 1) % 2 == 0) // and k%2==0 means saturation flow rate is 2 sec/veh
                {
                    Q_temp[i][phase1 - 1] -= LaneNo[phase1 - 1]; // discharging rate is based on the
                                                                 // number of the lane, assuming
                                                                 // queue at each lane are equal
                    // if (phase1==2 || phase1==6)
                    // Q_temp[i][phase1-1]-=3;  //queue discharge rate is 3 veh/2second for through
                    // phase 2 and 6, 2 veh/2second for phase 4 and 8, lanes and 1 veh/2 second for
                    // left turn phases
...
float CopDualRing::f1_r(int ring,
    int phase1,
    int phase2,
    int g1,
    int g2,
    ...
    float Num_Ratio[8])
{
    ...
    for (i = 4 * ring; i < 4 + 4 * ring; i++)
    {
        ...
        total_pre_delay[i] = Q_temp[total_length - B_L][i] *
                             (red_elapse_time[i] + total_length - B_L) /
                             2; // red_elapse_time+the first stage time
    }
    ...
    float Num_Ratio[8])
{
    ...
    float total_pre_delay_P1 = Q_temp[0][phase1 - 1] * red_elapse_time[phase1 - 1] /
                               2; // previous delay caused by queuing vehicle before planning

    float total_pre_delay_P2 = Q_temp[0][phase2 - 1] * red_elapse_time[phase2 - 1] /
                               2; // previous delay caused by queuing vehicle before planning
    NOTUSED(total_pre_delay_P1);
    NOTUSED(total_pre_delay_P2);
    ...
    for (i = 1; i <= total_length; i++)
    {
        if (i <= g1) // phase 1 is green
        {
            ...
            {
                Q_temp[i][phase1 - 1] -= LaneNo[phase1 - 1]; // discharging rate is based on the
                                                             // number of the lane, assuming queue
                                                             // at each lane are equal
                // if (phase1==2 || phase1==6)
                // Q_temp[i][phase1-1]-=3;  //queue discharge rate is 3 veh/2second for through
                // phase 2 and 6, 2 veh/2second for phase 4 and 8, lanes and 1 veh/2 second for left
                // turn phases
                // else if (phase1==4 || phase1==8)
                // Q_temp[i][phase1-1]-=2;
                // else
        ...
        if (i > g1 + yellow[phase1 - 1] + red[phase1 - 1] &&
            i <= g1 + yellow[phase1 - 1] + red[phase1 - 1] + g2) // phase 2 is green
        {
            Q_temp[i][phase1 - 1] = Q_temp[i - 1][phase1 - 1];
            Q_temp[i][phase2 - 1] = Q_temp[i - 1][phase2 - 1];
            // deal with the green phase
    ...
    if (Num_Veh[phase2 - 1] != 0)
        ...
        // if (Q_temp[g1+yellow[phase1-1]+red[phase1-1]+g2][phase2-1]!=0)   //add residual queue
        // penelty
    ...
    int ring, int phase1, int phase2, int total_length, int Num_Veh[8], float Num_Ratio[8])
{
    ...
    for (i = 4 * ring; i < 4 + 4 * ring; i++)
    {
        if ((i + 1) != phase1 && (i + 1) != phase2)
        {
            ...
            if (Num_Veh[i] != 0)
            {
                ...
                // cout << Ave_Delay_Phase[i] << " = " << T_Delay_Phase[i] << " / " << Num_Veh[i] <<
                // endl;
    ...
    // char temp_log[512];
    // sprintf(temp_log,"Total_Delay %d Total_Ave_Delay is: %f, ring %d phase1 %d phase2 %d
    // total_length %d Ave_Delay_Phase %f %f %f %f %f %f %f
    ...
    // cout<<temp_log;
    // sprintf(temp_log,"Num Vehicles %d %d %d %d %d %d %d
    // %d\n",Num_Veh[0],Num_Veh[1],Num_Veh[2],Num_Veh[3],Num_Veh[4],Num_Veh[5],Num_Veh[6],Num_Veh[7]);
    // cout<<temp_log;
...
float CopDualRing::f1_other(int ring,
    int phase1,
    int phase2,
    int total_length,
    int B_L,
    ...
    float Num_Ratio[8])
{
    ...
    for (i = 4 * ring; i < 4 + 4 * ring; i++)
    {
        if ((i + 1) != phase1 && (i + 1) != phase2)
        {
            Q_temp[total_length - B_L][i] = Q_perm[stage - 1][total_length - B_L][i];
            total_pre_delay[i] =
                Q_temp[total_length - B_L][i] * (red_elapse_time[i] + total_length - B_L) / 2;
            for (int j = total_length - B_L + 1; j <= total_length; j++)
            {
                Q_temp[j][i] = Q_temp[j - 1][i] + ArrivalTable[j][i];
                TotalDelay += Q_temp[j][i];
    ...
    float Num_Ratio[8])
{
    ...
    float total_pre_delay_P1 = Q_temp[total_length - B_L][phase1 - 1] *
                               (red_elapse_time[phase1 - 1] + total_length - B_L) /
                               2; // previous delay caused by queuing vehicle before planning

    float total_pre_delay_P2 = Q_temp[total_length - B_L][phase2 - 1] *
                               (red_elapse_time[phase2 - 1] + total_length - B_L) /
                               2; // previous delay caused by queuing vehicle before planning
    NOTUSED(total_pre_delay_P1);
    NOTUSED(total_pre_delay_P2);
    ...
    for (i = total_length - B_L + 1; i <= total_length; i++)
    {
        ...
        if (i > total_length - B_L + g1 && i <= g1 + total_length - B_L + yellow[phase1 - 1] +
                                                    red[phase1 - 1]) // transition time in phase 1
        {
            Q_temp[i][phase1 - 1] = Q_temp[i - 1][phase1 - 1] + ArrivalTable[i][phase1 - 1];
            Q_temp[i][phase2 - 1] = Q_temp[i - 1][phase2 - 1] + ArrivalTable[i][phase2 - 1];
            TotalDelay += Q_temp[i][phase1 - 1] + Q_temp[i][phase2 - 1];
        ...
        if (i > g1 + total_length - B_L + yellow[phase1 - 1] + red[phase1 - 1] &&
            i <= g1 + total_length - B_L + yellow[phase1 - 1] + red[phase1 - 1] +
                     g2) // phase 2 is green
        {
            Q_temp[i][phase1 - 1] = Q_temp[i - 1][phase1 - 1];
            Q_temp[i][phase2 - 1] = Q_temp[i - 1][phase2 - 1];
        ...
        if (i > g1 + total_length - B_L + yellow[phase1 - 1] + red[phase1 - 1] + g2 &&
            i <= total_length) // all red
        {
            Q_temp[i][phase1 - 1] = Q_temp[i - 1][phase1 - 1] + ArrivalTable[i][phase1 - 1];
            Q_temp[i][phase2 - 1] = Q_temp[i - 1][phase2 - 1] + ArrivalTable[i][phase2 - 1];
            TotalDelay += Q_temp[i][phase1 - 1] + Q_temp[i][phase2 - 1];
    ...
    if (Num_Veh[phase2 - 1] != 0)
        ...
        // if (Q_temp[g1+total_length-B_L+yellow[phase1-1]+red[phase1-1]+g2][phase2-1]!=0)   //add
        // residual queue penelty
        Ave_Delay_P2 += Q_temp[total_length][phase2 - 1];
...
int CopDualRing::add_missing_phase(int barrier)
{
