...
class Lane
{
  public:
    ...
    int Direction; // This is direction of the lane
    // Northbound=1;Eastbound=2;Southbound=3;Westbound=4; reserved! not use now
    int Node_No;
...
class MAP
{
  public:
    ...
    // method
    bool ParseIntersection(const char* filename);

    int getLaneNum(int laneID) const;
    int getLaneID(int approach, int lane) const;
    int getApproach(int laneID) const;
};
