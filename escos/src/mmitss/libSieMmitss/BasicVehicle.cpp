...
// Create an empty valid buffer for BSM blob and copy it to the imported buffer.
void BasicVehicle::CreateEmptyBSMblob(char* theTarget)
{
    ...
    {
        std::cout << "ERROR: Length of the blank message is not correct.!" << std::endl;
        std::cout << "Expected to find " << BSM_BlOB_SIZE << " but found " << sizeof(emptyMsg)
                  << " bytes." << std::endl;
    }
    ...
    {
        std::cout << "ERROR: passed a null pointer to the function CreateEmptyBSMblob."
                  << std::endl;
    }
}

// Unpack the BSM blob stream and populates the vehicle attributes (the class data)
void BasicVehicle::BSMToVehicle(char* ablob)
{
    if (ablob == NULL)
    {
        std::cout << "Passed in an empty pointer for BSM char char array, no BSMToVehicle decode."
                  << std::endl;
        return;
    }
    ...
    // the code is written to be compatible with Intel platforms (which are all little endian)
    ...
    offset += 1; // move past to next item
    ...
    lTemp = UnpackBlob(offset, ablob, 4);
    pos.latitude = ((unsigned long)lTemp) / DEG2mDEG; // convert and store as float
    offset += 4;
    ...
    // There is no elevation data element in VISSIM, so this may just be a default constant each
    // time we use
    lTemp = UnpackBlob(offset, ablob, 2);
    pos.elevation = ((unsigned long)lTemp) / 10.0; // in 0.1 m steps
    offset += 2;
...
void BasicVehicle::VehicleToBSM(char* ablob)
{
    if (ablob == NULL)
    {
        std::cout << "Passed in an empty pointer for BSM to the VehicleToBSM function" << std::endl;
        return;
    }
    ...
    pByte = (char*)&usTemp;
    ablob[offset + 0] = (*(pByte + 1)) | theTrans;
    ablob[offset + 1] = *pByte;
    offset += 2;
    ...
    lTemp = ((((long)((width + 0.005f) * 100.0f)) & 0x03FF) << 14) +
            (((long)((length + 0.005f) * 100.0f)) & 0x3FFF);
    pByte = (char*)&lTemp;
    FillupBlob(offset, ablob, pByte, 3);
    offset += 3;

    assert(offset == BSM_BlOB_SIZE); // check if we exactly used 38 byte of blob
}
