...
std::vector<int> GeneratePhaseArray(
    int StartPhase, const std::vector<int>& PhaseSeq, int N, int TotalNo, int NoRepeat)
{
    if (NoRepeat == 0)
    {
        // N is the phase number in a cycle for a ring, so for 8 phase, it is 4, TotalNo is the
        // total number of phases in this sequence.
        int idx = FindIndexArray(PhaseSeq, N, StartPhase);
        int Cycle = TotalNo / N + 1;
        std::vector<int> PhaseArray1(N * Cycle, 0);
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < Cycle; j++)
            {
        ...
        std::vector<int> ArrayReturn(TotalNo, 0);
    ...
    else //
    {
        ...
        int idx = FindIndexArray(PhaseSeq, N, StartPhase);
        int nCycles = 4;
        std::vector<int> phase_array(nCycles * N, 0); // At most n cycles, each cycle has N elements
