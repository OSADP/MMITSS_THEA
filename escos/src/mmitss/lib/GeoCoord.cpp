...
void geoCoord::ecef2local(double ex, double ey, double ez, double* x, double* y, double* z)
{
    ...
    *x = (-R_sinLat * R_cosLong) * (ex - ex_init) + (-R_sinLat * R_sinLong) * (ey - ey_init) +
         R_cosLat * (ez - ez_init);
    *y = (-R_sinLong) * (ex - ex_init) + (R_cosLong) * (ey - ey_init) + 0;
    *z = (-R_cosLat * R_cosLong) * (ex - ex_init) + (-R_cosLat * R_sinLong) * (ey - ey_init) +
         (-R_sinLat) * (ez - ez_init);
}

void geoCoord::lla2ecef(
    double longitude, double latitude, double altitude, double* x, double* y, double* z)
{
    ...
    // compute constants
    double N = a / sqrt(1.0l - pow(e * sin(latitude_r), 2));
